...
// Constructor
ServoOsc::ServoOsc(int initAngle, uint16_t p, uint8_t a, int8_t o, int8_t ph, int8_t tr,
                   int8_t pn, bool atch):
    initAngle(initAngle),
    period(p),
    amplitude(a),
...
// a: 0-180
bool ServoOsc::positionServo(int16_t a, bool t) {
    ...
    if (!usePwmDriver)
        ...
        uint16_t target_angle = map(a, 0, 180, SERVO_MIN_PULSE_CNT, SERVO_MAX_PULSE_CNT);
        ...
        pwmDriver.setPWM(pwmPin, 0, target_angle);
    }
...
void ServoOsc::update() {
    ...
    if (!stopped) {
        ...
        if (!usePwmDriver)
            ...
            pwmDriver.setPWM(pwmPin, 0, map(pos + 90 + trim, 0, 180, SERVO_MIN_PULSE_CNT, SERVO_MAX_PULSE_CNT));
        angle = pos + 90;
#ifdef __SO_DBG
