...
#define SERVO_MIN_PULSE_CNT 90
#define SERVO_MAX_PULSE_CNT 470
...
private:
    ...
    int8_t initAngle=90;   // Initial angle to set the servo to when attached
    int angle=90;       // Current angle of the servo

    Adafruit_PWMServoDriver pwmDriver;  // The PWM driver object
    bool usePwmDriver=false;            // True if using the PWM driver
    uint8_t pwmPin;                    // The PWM pin to use on the driver
...
public:
    // Constructor
    ServoOsc(int initAngle=90, uint16_t p=2000, uint8_t a=45, int8_t o=0, int8_t ph=0,
             int8_t trim=0, int8_t pn=-1, bool atch=false);
    void registerDriver(Adafruit_PWMServoDriver &pwm, uint8_t p) {pwmDriver = pwm; usePwmDriver=true; pwmPin=p;};
    ...
    bool positionServo(int16_t angle, bool withTrim=true);
